...
if($type === "png"){
  ...
  if($result === false){
    trigger_error("generateAvatar.php died on imagealphablending png");
    die();
  }
  $result=imagesavealpha($newImg, true);
  if($result === false){
    trigger_error("generateAvatar.php died on imagesavealpha png");
    die();
  }
}
...
if($size === false){
  trigger_error("generateAvatar.php died on filesize");
  die();
}
